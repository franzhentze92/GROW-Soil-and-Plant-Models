# -*- coding: utf-8 -*-
"""
Created on Thu Feb 20 12:45:19 2025
...
def main():
    ...
    # Ask the user to select a single product
    print("Select a product:")
    product = get_user_input("Choose a product:", product_options)

    # Ask for the amount in kg
    while True:
        try:
    ...
    # Display individual nutrient breakdown
    print("\nNutrient Breakdown:")
    results = calculate_nutrient_breakdown(product, amount)
    print(f"{'Nutrient':<15}{'kg/ha':<12}{'ppm':<12}")
    for row in results:
        print(f"{row['Nutrient']:<15}{row['kg/ha']:<12.4f}{row['ppm']:<12.4f}")
