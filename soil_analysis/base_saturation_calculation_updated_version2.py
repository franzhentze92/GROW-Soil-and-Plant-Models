# -*- coding: utf-8 -*-
"""
Created on Wed Apr 23 14:05:00 2025
...
given_values = {
    "ca_cmol": 8.9,
    "mg_cmol": 2.69, # 2.66 was not correct, the database contains 2.69
    "k_cmol": 0.66,
    "na_cmol": 0.17,
    "al_cmol": 0.06,
    "ph": 6.50, # 6.49 is what should be used, but the matrix doesn't have it, so it uses 6.50
...
closest_pH = min(ph_matrix.keys(), key=lambda x: abs(x - given_values["ph"]))
...
# Calculate Other Bases %, then adjust if total > 100%
...
# Correction: ensure base saturation doesn't exceed 100%
total = sum(base_saturation.values())
if total > 100:
    surplus = total - 100
    if base_saturation["Other_Bases"] >= surplus:
        base_saturation["Other_Bases"] -= surplus
    else:
        surplus -= base_saturation["Other_Bases"]
        base_saturation["Other_Bases"] = 0
        base_saturation["H"] = max(0, base_saturation["H"] - surplus)
